...
Forms are mostly rectangular and sheet-like. Figure frames, buttons, and certificate tiles use a modest 7px corner; project and education panels use 12px; 18px is retained as a large-system option but is not a dominant silhouette. Metadata tags alone use the full pill radius. Borders are one pixel, never ornamental double strokes; the portrait receives one inset rule to suggest a mounted plate.
